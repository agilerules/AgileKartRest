...
import org.AgileKartRest.model.ProductReview;
...
public class ProductReviewEndpoint
{
   ...
   {
      TypedQuery<ProductReview> findByIdQuery = em.createQuery("SELECT DISTINCT p FROM ProductReview p LEFT JOIN FETCH p.product LEFT JOIN FETCH p.users WHERE p.productReviewId = :entityId ORDER BY p.productReviewId", ProductReview.class);
   ...
   {
      TypedQuery<ProductReview> findAllQuery = em.createQuery("SELECT DISTINCT p FROM ProductReview p LEFT JOIN FETCH p.product LEFT JOIN FETCH p.users ORDER BY p.productReviewId", ProductReview.class);
   ...
   @Path("/{id}")
   @Consumes("application/json")
   public Response update(@PathParam("id") String id, ProductReview entity)
   {
      if (entity == null)
      {
         return Response.status(Status.BAD_REQUEST).build();
      }
      if (!id.equals(entity.getProductReviewId()))
      {
