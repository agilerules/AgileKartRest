...
import org.AgileKartRest.model.Tax;
...
public class TaxEndpoint
{
   ...
   {
      TypedQuery<Tax> findByIdQuery = em.createQuery("SELECT DISTINCT t FROM Tax t LEFT JOIN FETCH t.taxRules WHERE t.taxId = :entityId ORDER BY t.taxId", Tax.class);
   ...
   {
      TypedQuery<Tax> findAllQuery = em.createQuery("SELECT DISTINCT t FROM Tax t LEFT JOIN FETCH t.taxRules ORDER BY t.taxId", Tax.class);
   ...
   @Path("/{id}")
   @Consumes("application/json")
   public Response update(@PathParam("id") String id, Tax entity)
   {
      if (entity == null)
      {
         return Response.status(Status.BAD_REQUEST).build();
      }
      if (!id.equals(entity.getTaxId()))
      {
         return Response.status(Status.CONFLICT).entity(entity).build();
      }
      if (em.find(Tax.class, id) == null)
      {
         return Response.status(Status.NOT_FOUND).build();
      }
      try
