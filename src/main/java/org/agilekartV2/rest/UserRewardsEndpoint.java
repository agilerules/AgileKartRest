...
import org.agilekartV2.model.UserRewards;
...
public class UserRewardsEndpoint
{
   ...
   {
      TypedQuery<UserRewards> findByIdQuery = em.createQuery("SELECT DISTINCT u FROM UserRewards u WHERE u.rewardId = :entityId ORDER BY u.rewardId", UserRewards.class);
   ...
   {
      TypedQuery<UserRewards> findAllQuery = em.createQuery("SELECT DISTINCT u FROM UserRewards u ORDER BY u.rewardId", UserRewards.class);
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Consumes("application/json")
   public Response update(UserRewards entity)
   {
      try
      {
