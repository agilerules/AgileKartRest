...
import org.agilekartV2.model.PaymentGateway;
...
public class PaymentGatewayEndpoint
{
   ...
   {
      TypedQuery<PaymentGateway> findByIdQuery = em.createQuery("SELECT DISTINCT p FROM PaymentGateway p WHERE p.paymentGatewayId = :entityId ORDER BY p.paymentGatewayId", PaymentGateway.class);
   ...
   {
      TypedQuery<PaymentGateway> findAllQuery = em.createQuery("SELECT DISTINCT p FROM PaymentGateway p ORDER BY p.paymentGatewayId", PaymentGateway.class);
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Consumes("application/json")
   public Response update(PaymentGateway entity)
   {
      try
      {
