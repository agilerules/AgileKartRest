...
import org.agilekartV2.model.MerchantCategory;
...
public class MerchantCategoryEndpoint
{
   ...
   {
      TypedQuery<MerchantCategory> findByIdQuery = em.createQuery("SELECT DISTINCT m FROM MerchantCategory m WHERE m.merchantCategoryId = :entityId ORDER BY m.merchantCategoryId", MerchantCategory.class);
   ...
   {
      TypedQuery<MerchantCategory> findAllQuery = em.createQuery("SELECT DISTINCT m FROM MerchantCategory m ORDER BY m.merchantCategoryId", MerchantCategory.class);
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Consumes("application/json")
   public Response update(MerchantCategory entity)
   {
      try
      {
