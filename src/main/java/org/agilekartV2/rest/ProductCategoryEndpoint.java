...
import org.agilekartV2.model.ProductCategory;
...
public class ProductCategoryEndpoint
{
   ...
   {
      TypedQuery<ProductCategory> findByIdQuery = em.createQuery("SELECT DISTINCT p FROM ProductCategory p WHERE p.productCategoryId = :entityId ORDER BY p.productCategoryId", ProductCategory.class);
   ...
   {
      TypedQuery<ProductCategory> findAllQuery = em.createQuery("SELECT DISTINCT p FROM ProductCategory p ORDER BY p.productCategoryId", ProductCategory.class);
   ...
   @Path("/{id:[0-9][0-9]*}")
   @Consumes("application/json")
   public Response update(ProductCategory entity)
   {
      try
      {
