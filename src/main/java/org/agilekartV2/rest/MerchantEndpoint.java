...
import org.agilekartV2.model.Merchant;
...
public class MerchantEndpoint
{
   ...
   {
      TypedQuery<Merchant> findByIdQuery = em.createQuery("SELECT DISTINCT m FROM Merchant m WHERE m.merchantId = :entityId ORDER BY m.merchantId", Merchant.class);
   ...
   {
      TypedQuery<Merchant> findAllQuery = em.createQuery("SELECT DISTINCT m FROM Merchant m ORDER BY m.merchantId", Merchant.class);
